...
class CreateIndicatorProfile extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('indicator_profiles', function (Blueprint $table) {
            ...
            $table->uuid('first_pic_id');
            $table->uuid('second_pic_id')->nullable();
            $table->softDeletes();
            $table->timestamps();
            ...
            $table->foreign('first_pic_id')
            ->references('id')
            ->on('users')
            ->onUpdate('cascade')->onDelete('restrict');

            $table->foreign('second_pic_id')
            ->references('id')
            ->on('users')
            ->onUpdate('cascade')->onDelete('restrict');
        });
    ...
    {
        Schema::dropIfExists('indicator_profiles');
    }
}
